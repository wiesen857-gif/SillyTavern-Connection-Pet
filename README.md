...
拓展会自动列出酒馆连接管理器中所有属于 Chat Completion → Custom 的配置，尚未保存 API 地址或模型的配置也不会被隐藏。酒馆现有配置可以直接编辑并获取模型；点击“保存配置”后，名称、API 地址、模型和 Secret ID 会同步更新到酒馆连接管理器，原配置中的预设、代理等其他字段保持不变。也可以将它复制为桌宠独立配置。
...
1. 验证预设和条目 ID；
2. 加载所选预设；
3. 将本次勾选的待开启条目设为开启，未勾选的待开启条目设为关闭，并保存到该命名预设；
4. 立即刷新提示词管理器。
